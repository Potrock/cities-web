	...
	if (isLoading || !serverData.image) {
		return (
			<div className="card bg-base-100 shadow-m">
				...
				<div className="card-body">
					<h2 className="card-title">City #{tokenId}</h2>
					<p>Loading...</p>
				</div>
			</div>
		);
		...
		return (
			<div className="card bg-base-200 shadow-m">
				...
				<div className="card-body">
					<h2 className="card-title">City #{serverData.id}</h2>
					{/* <p>Number of Houses: {serverData.houseCount}</p> */}
					<p>Number of Farms: {serverData.farmCount}</p>
					<div className="flex justify-center">
						<Link href={`/cities/${serverData.id}`}>
							{owned && <button className="btn btn-primary">Manage</button>}
							{!owned && <button className="btn">Visit</button>}
						</Link>
					</div>
				</div>
			</div>
